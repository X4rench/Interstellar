...
app.post(
  ...
  if (userInfo && typeof userInfo === 'object') {
    ...
    const age = Number.isInteger(userInfo.age) && userInfo.age >= 1 && userInfo.age <= 120
      ...
      : null;
    const parts = [];
    if (gender) parts.push(gender === 'male' ? 'мужского пола' : gender === 'female' ? 'женского пола' : 'пол не указан');
    if (age) parts.push(`${age} ${age === 1 ? 'год' : age >= 2 && age <= 4 ? 'года' : 'лет'}`);
    if (parts.length > 0) {
      // Инструкция модели — естественно использовать инфу для согласования
      // рода и тональности. НЕ зачитывать вслух, НЕ делать темой разговора.
      userInfoBlock =
        `\n\n[Информация о собеседнике для контекста]\n` +
        `Ты общаешься с человеком ${parts.join(', ')}. ` +
        `Учитывай это при общении: правильно согласуй грамматический род, ` +
        `подбирай адекватную возрасту тональность. ` +
