      ...
      if (text.startsWith('/paysupport')) {
        ...
        sendBotMessage({
          ...
          text:
            ...
            `Тарифы:\n` +
            `• Basic — 300 ₽ / мес (50 сообщений в день)\n` +
            `• Premium — 750 ₽ / мес (200 сообщений + 18+)\n` +
            `• Day Pass — 75 ₽ разово (24h без лимита)\n\n` +
            ...
            `2️⃣ <b>Деньги списались, но подписки нет</b> — напиши на ${supportEmail} с указанием своего Telegram user_id и приложи скриншот списания из приложения банка.\n\n` +
            `3️⃣ <b>Хочешь возврат</b> — по ЗоЗПП ст. 32 ты вправе отказаться от подписки. Напиши на ${supportEmail} с указанием user_id, и мы вернём средства на ту же карту, с которой была оплата. Срок возврата ЮKassa — до 10 рабочих дней.\n\n` +
            `4️⃣ <b>Отмена автопродления</b> — открой Mini App → Профиль → Подписка → «Отключить автопродление». Подписка останется активна до конца оплаченного периода.\n\n` +
            `5️⃣ <b>Day Pass возврату не подлежит</b> после начала использования (цифровой контент с немедленным предоставлением, ПП РФ № 55).\n\n` +
