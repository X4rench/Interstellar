  ...
  try {
    ...
    if (text && chatId) {
      if (text.startsWith('/paysupport')) {
        ...
        const supportEmail = process.env.SUPPORT_EMAIL || 'apppartners@mail.ru';
        sendBotMessage({
          chatId,
          text:
            `💳 <b>Поддержка по платежам — Интерстеллар</b>\n\n` +
            `<b>Тарифы:</b>\n` +
            `• Free — 0 ₽. 10 сообщений в день, базовый набор персонажей без 18+\n` +
            `• Basic — 300 ₽ / мес. 50 сообщений в день, все персонажи без 18+\n` +
            `• Premium — 750 ₽ / мес. 200 сообщений в день, доступ к персонажам с маркером 18+, расширенная память диалогов\n` +
            `• Day Pass — 75 ₽ разово. +100 сообщений к дневному лимиту любого тарифа на 24 часа (Free → 110, Basic → 150, Premium → 300)\n\n` +
            `Оплата через ЮKassa: банковской картой, СБП или иными способами, поддерживаемыми ЮKassa.\n\n` +
            `<b>Если что-то пошло не так:</b>\n\n` +
            `1️⃣ <b>Подписка не активировалась</b> — подожди 5 минут (webhook от ЮKassa может задержаться) и перезайди в приложение через Menu Button.\n\n` +
            `2️⃣ <b>Деньги списались, но подписки нет</b> — напиши на ${supportEmail} с указанием Telegram user_id и приложи скриншот списания из приложения банка.\n\n` +
            `3️⃣ <b>Отмена автопродления</b> — открой Mini App → Профиль → Подписка → «Отключить автопродление». Подписка останется активна до конца уже оплаченного периода (деньги за оплаченный месяц автоматически не возвращаются).\n\n` +
            `4️⃣ <b>Досрочный возврат подписки (ст. 32 ЗоЗПП)</b> — рассчитывается пропорционально неиспользованной части подписки по формуле:\n\n` +
            `<i>Возврат = Стоимость × min(оставшиеся дни / 30; оставшийся лимит сообщений / месячный лимит) − комиссия ЮКассы (≈ 3%)</i>\n\n` +
            `Пример: купил Basic (300 ₽), через 5 дней использовал 200 сообщений из 1500 → к возврату ≈ 241 ₽. Возврат — на ту же карту, в течение 10 рабочих дней.\n\n` +
            `Заявку отправь на ${supportEmail} с указанием Telegram user_id.\n\n` +
            `5️⃣ <b>Day Pass возврату не подлежит</b> после активации (отправки первого сообщения сверх дневного лимита) — ПП РФ № 55, п. 11. До активации — возврат в полном объёме.\n\n` +
            `6️⃣ <b>Лимит на досрочные возвраты</b> — не более одного раза в течение 12 месяцев на пользователя. Ограничение не применяется при некачественном оказании услуги.\n\n` +
            `Полные условия — в Mini App → Профиль → Юридические документы → «Условия подписки и возврата».`,
        });
        return res.status(200).end();
      }
...
app.post(
  ...
  // Юзер заполнил «о себе» в Профиле: пол/возраст. Аккуратно валидируем и
  // форматируем небольшой блок для system-prompt чтобы LLM знала
  // тональность и грамматический род. Без user_info — старое поведение.
  let userInfoBlock = '';
  if (userInfo && typeof userInfo === 'object') {
    ...
    if (parts.length > 0) {
      // Инфа о собеседнике для согласования рода/тональности. Ключевой момент:
      // модель НЕ должна выдавать, что эти данные ей «дали» (анкета/профиль/
      // «ты сам написал») — это ломает образ. Если юзер прямо спросит — отвечать
      // в роли, как собственное впечатление/догадку, а не зачитку из формы.
      userInfoBlock =
        ...
        `Если он прямо спросит, что ты знаешь про его пол или возраст — ответь в характере, как догадку или наблюдение («рискну предположить…», «ты производишь впечатление…», «по тому, как ты пишешь, мне кажется…»), а не как зачитанный факт.`;
    }
  }
