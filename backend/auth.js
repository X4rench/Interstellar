...
/**
 * TTL валидности initData. Защита от replay-атак с украденным initData.
 *
 * Было 24h — слишком мало: Telegram Desktop и web.telegram.org иногда
 * выставляют auth_date моментом загрузки страницы, а не открытия мини-апп.
 * Пользователь, оставивший Telegram Desktop открытым на сутки, получал 401
 * и видел "Free тариф" несмотря на активную подписку.
 *
 * 7 дней — безопасный компромисс: украденный initData всё равно даёт доступ
 * только к аккаунту жертвы (не к чужим деньгам), а легитимные юзеры не
 * выбрасываются.
 */
const INIT_DATA_TTL_MS = 7 * 24 * 60 * 60 * 1000; // 7 дней
